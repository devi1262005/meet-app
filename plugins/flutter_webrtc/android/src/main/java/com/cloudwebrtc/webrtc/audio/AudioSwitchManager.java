    ...
    private void initAudioSwitch() {
        if (audioSwitch == null) {
            ...
            handler.postAtFrontOfQueue(() -> {
                // Check if the device is running API level 23 or higher
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    ...
                    // Handle the case when the device is below API 23 (if necessary)
                    // Maybe show a fallback or just don't initialize the audio switch
                    Log.w("AudioSwitch", "AudioSwitch requires API 23 or higher");
                }
            });
        }
    }
